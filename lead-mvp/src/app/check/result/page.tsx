...
export default function ResultPage() {
  ...
  return <>
    ...
    <main id="main" className="result-content">
      ...
      <div className="result-states single-result" aria-label={`התוצאה שלך: ${state.label}`}>
        <article className={`result-state single-state ${result} active`} aria-current="true"><span className="state-badge">{state.label}</span><h2>{state.title}</h2><p>{state.text}</p></article>
      </div>
      ...
      <div className="legal-notice"><strong>חשוב לדעת</strong><span>עבר פלילי, צו הרחקה בתוקף או מגבלה רפואית עשויים להשפיע על הבקשה. אין צורך למסור מידע כזה באתר. ניתן לדון בכך ישירות עם עורך דין בשיחה.</span></div>
      <h2 className="result-cta-heading">רוצים לבדוק את המקרה לעומק?</h2>
