...
export default async function CockpitDashboardPage() {
  ...
  return <main className="p-5 sm:p-8 lg:p-10">
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <section className="rounded-[26px] border border-white/10 bg-white/5 p-5 sm:p-6"><h2 className="font-display text-2xl font-black">תיקים אחרונים</h2><div className="mt-4 divide-y divide-white/10">{cases.map((item)=><Link key={item.id} href={`/cockpit/cases/${item.id}`} className="flex items-center justify-between gap-4 py-4"><div><strong>{item.case_number}</strong><p className="mt-1 text-xs text-slate-400">{item.title}</p></div><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold">{caseStatusLabel(item.status)}</span></Link>)}</div></section>
