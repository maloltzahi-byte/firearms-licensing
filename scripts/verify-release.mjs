...
if (contracts.count !== 98 || contracts.contracts?.length !== 98) fail.push(`contracts:${contracts.contracts?.length ?? 'missing'}`)
...
for (const moduleName of modules) {
  if (!fs.existsSync(path.join(root, `stage11/modules/${moduleName}.html`))) fail.push(`stage11:${moduleName}`)
  if (!fs.existsSync(path.join(root, `public/canonical/${moduleName}.html`))) fail.push(`runtime:${moduleName}`)
}
...
console.log('RELEASE VERIFY PASS: 98 contracts / 6 Stage 11 modules / 6 runtime canonical modules / 45 routes / Next 15.5.24 / React 19.2.6')
